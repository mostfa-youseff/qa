...
class GitService {
  Future<String> cloneRepo(String repoUrl, {String? token}) async {
    ...
    if (!isPublic && token != null) {
      cloneUrl = repoUrl.replaceFirst('https://', 'https://oauth2:$token@');
    } else if (!isPublic) {
  ...
  Future<bool> isPublicRepo(String repoUrl) async {
    try {
      final response = await http.head(Uri.parse(repoUrl));
      return response.statusCode == 200;
    } catch (e) {
      return false; // Assume private if request fails (e.g., 401/403)
    }
  }
}
